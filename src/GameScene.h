...
//标杆，进度尺
class FlagMeter: public QGraphicsPixmapItem // ||Graphics(图样)， Pixmap（像素图）
{
public:
...
private:
    ...
    QGraphicsPixmapItem *background, *gamelose;
    QGraphicsItemGroup *gameGroup;
    QGraphicsSimpleTextItem *infoText;
    QGraphicsRectItem *infoTextGroup;
    //菜单按钮
    MouseEventPixmapItem *menuGroup, *menuBackButton, *menuBackGameButton;
    MouseEventPixmapItem *front1LevelButton, *back1LevelButton, *top1LevelButton;
    MouseEventPixmapItem *front2LevelButton, *back2LevelButton, *top2LevelButton;

    QGraphicsPixmapItem *menuSelect;
    ...
    //QGraphicsItemGroup 是一种特殊类型的复合项，
    //它将自身及其所有子项视为一个项（即所有子项的所有事件和几何图形都合并在一起）
